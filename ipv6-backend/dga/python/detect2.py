...
def filtertime(detail):
	...
	starttime = nxs.newtime[nxs.head(1).index[0]]
	endtime = nxs.newtime[nxs.tail(1).index[0]]
...
if __name__ == '__main__':
	# dga_20190602_1745.log
	# resp_20190329_17.log
	...
	if not os.path.exists('processedlog.txt'):
		fw = open('processedlog.txt','w')
	pd.set_option('display.max_columns', None)
	pd.set_option('display.max_rows', None)
	pd.set_option('display.width',1000)
	fw = open('result.txt','w') #init
	while True:
		fi = open('processedlog.txt')
		processedlog = []
		for f in fi:
			processedlog.append(f[:-1])
		fi.close()
		filenames = os.listdir('/log/result')
		filenames.sort()
		for filename in filenames:
			if filename in processedlog:
				continue
			try:
				respname = '/log/resp/resp' + filename[3:]
				fi = open(respname)
			except:
				print 'log not found in resp.'
				continue
			fi = open('/log/result/' + filename)
			...
			for f in fi:
				if f == '--------\n':
					...
					detail.drop(['time'],axis=1,inplace=True)
					ccs = filtertime(detail)
					if ccs.empty:
						continue
					print ccs
					fw.writelines(ip + str(domains) + '\n')
